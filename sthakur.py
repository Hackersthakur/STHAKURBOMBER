...
def check_for_updates():
    ...
    fver = requests.get("https://github.com/Hackersthakur/STHAKURBOMBER/blob/main/.version").text.strip()
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("An update is available")
        mesgdcrt.GeneralMessage("Starting update...")
        update()
    else:
        mesgdcrt.SuccessMessage("S THAKUR BOMBER is up-to-date")
        mesgdcrt.GeneralMessage("Starting THAKURBOMBER")

def notifyen():
    try:
        noti = requests.get("https://github.com/Hackersthakur/STHAKURBOMBER/blob/main/.new").text.upper().strip()
        if len(noti) > 10:
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit={"sms":500000,"call":1500,"mail":2000}
        cc,target="",""
        if mode in ["sms","call"]:
...
parser.add_argument("-mail","--mail", action="store_true",help="start THAKURBOMBER with MAIL Bomb mode")
