...
fapi = requests.get("https://raw.githubusercontent.com/Hackersthakur/STHAKURBOMBER/main/protection.txt").text.strip()
    ...
    try:
        ...
        while (protectbool):
            try:
                ...
                if len(protect) == 10:
                    p = []
                    if os.path.exists("protcookie.txt"):
                        with open ("protcookie.txt", "r") as coo:
                            prot = coo.read()
                            p = prot.split(",")
                            count = len(p)
                        ...
                        with open ("protcookie.txt", "a") as coos:
                            coos.write(","+protect)
                            coos.close()
                    else:
                        with open ("protcookie.txt", "w") as coo:
                            coo.write(protect)
                            coo.close()
